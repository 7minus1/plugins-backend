...
export class UsersController {
  ...
  async verifyCode(@Body() dto: VerifyCodeDto) {
    const isValid = await this.usersService.verifyCode(dto.phoneNumber, dto.code);
    if (!isValid) {
  ...
  @Get('vip-status')
  @ApiBearerAuth()
  @ApiOperation({ summary: '获取VIP状态', description: '获取当前用户的VIP状态和到期时间' })
  @ApiResponse({ 
    status: 200, 
    description: '获取成功',
    schema: {
  ...
  @Get('upload-count')
  @ApiBearerAuth()
  @ApiOperation({ summary: '获取上传次数', description: '获取当前用户的上传次数信息' })
  @ApiResponse({ 
    status: 200, 
    description: '获取成功',
    schema: {
